...
### Agent roster — how many agents, and what each is responsible for

**Six agents ship today, plus one orchestrator. A seventh (`classify`) is specified but not built ([§10.1](#101-planned-classify-stage--identify-the-document-before-extracting)).**

An "agent" here is a single-responsibility stage module behind a narrow function interface — not an autonomous LLM loop. Only **one** of the six calls a model; the other five are deterministic Python. They are pure functions of their inputs, so any agent can be swapped, stubbed or re-run in isolation without touching the orchestrator or the UI.

| #    | Agent           | Module                                                                                                      | Entry point                                | Responsibility (one job only)                                                                                                                                                                                                                  | Uses the LLM? |
| ---- | --------------- | ----------------------------------------------------------------------------------------------------------- | ------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------- |
| 1    | **Forensics**   | [`forensics.py`](Web-api/app/pipeline/forensics.py)                                                         | `analyze()`                                | Decide whether the **bytes** were tampered with: SHA-256 dedupe, magic-byte vs extension, PDF metadata/overlays/JS, EXIF editor tags, Error Level Analysis. Emits weighted, human-readable signals — never a verdict.                          | No            |
| 2    | **Read**        | [`docread.py`](Web-api/app/pipeline/docread.py)                                                             | `read_document()`                          | Turn any container into something a model can consume: native PDF text layer, DOCX/XLSX cell text, or rendered base64 PNG page images. Sets `readable=False` when nothing can be extracted.                                                    | No            |
| 3    | **Extraction**  | [`llm.py`](Web-api/app/pipeline/llm.py) + [`prompts.py`](Web-api/app/pipeline/prompts.py)                   | `extract_fields()`                         | Ask the vision model for the fields the document type declares, as strict JSON (`format=json`, `temperature=0`). Owns prompt construction, the response cache, the concurrency semaphore, one retry on malformed JSON, and a confidence value. | **Yes**       |
| 4    | **Rules**       | [`rules.py`](Web-api/app/pipeline/rules.py) + [`document_rules.py`](Web-api/app/pipeline/document_rules.py) | `evaluate()`                               | Judge the extracted fields against the doc-type rule set: expiry windows, required-field presence, type-specific guards. Emits per-rule pass/fail with a weight and a reason.                                                                  | No            |
| 5    | **Field match** | [`field_match.py`](Web-api/app/pipeline/field_match.py)                                                     | `compare()`                                | Confirm the document belongs to **this applicant**: RapidFuzz name comparison with aliases, DOB/address normalisation, per-field similarity score. `+10` risk per mismatch.                                                                    | No            |
| 6    | **Score**       | [`score.py`](Web-api/app/pipeline/score.py)                                                                 | `aggregate()`                              | The only agent allowed to decide. Sums rule weights + `10×mismatches` + forensics weights (clamped 0..100), takes worst-of the three statuses, applies `RISK_PASS_MAX` / `RISK_FAIL_MIN`, and returns the final `PipelineOutput`.              | No            |
| —    | _Orchestrator_  | [`runner.py`](Web-api/app/pipeline/runner.py)                                                               | `run_pipeline()` / `run_pipeline_stream()` | Not an agent: sequences the six, times each stage, wraps failures in `PipelineStageError(stage, cause)`, short-circuits to `UNREADABLE` after agent 2, and emits one SSE `stage` frame per agent.                                              | No            |
| 7 🕒 | **Classify**    | _not built_                                                                                                 | —                                          | **Planned.** Identify the document type from the page itself before extraction, and gate the run when it contradicts (or is missing from) the user's selection. Would sit between agents 2 and 3 and cost a second model call.                 | Would be      |

Design rules that keep the roster honest:

- **One agent, one decision surface.** Forensics, rules and match each emit _signals_; only `score` converts signals into a status. Adding a status escalation anywhere else makes the outcome untraceable.
- **Only agent 3 is non-deterministic.** Everything else re-runs to the identical answer on the same bytes, which is what makes the audit JSONL replayable and the failure explainable to a reviewer.
- **`PIPELINE_STAGES` is the contract.** The tuple in [runner.py](Web-api/app/pipeline/runner.py) is mapped 1:1 onto the Web-UI progress component, so adding or renaming an agent is a coordinated front-end + back-end change.
- **Agents never call each other.** All data flows through the orchestrator, so a stage can be stubbed (`LLM_MODE=stub`, `DOC_READ_MODE=stub`) or lifted into its own queue worker ([§11.8](#118-worker-architecture)) without a rewrite.
